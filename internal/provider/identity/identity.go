	...
	return schema.SingleNestedAttribute{
		...
		Attributes: map[string]schema.Attribute{
			...
			"primary_key": schema.StringAttribute{
				MarkdownDescription: "Primary key, base64 encoded (16 to 64 bytes). Generated by the hub on create when omitted. A key " +
					"rotated outside Terraform is adopted on refresh unless you set it explicitly. Not returned when `primary_key_wo` is used.",
				Optional:  true,
				Computed:  true,
				Sensitive: true,
			},
			"secondary_key": schema.StringAttribute{
				MarkdownDescription: "Secondary key, base64 encoded (16 to 64 bytes). Generated by the hub on create when omitted. A key " +
					"rotated outside Terraform is adopted on refresh unless you set it explicitly. Not returned when `secondary_key_wo` is used.",
				Optional:  true,
				Computed:  true,
				Sensitive: true,
			},
