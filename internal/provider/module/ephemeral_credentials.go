...
func (e *credentialsEphemeral) Schema(_ context.Context, _ ephemeral.SchemaRequest, resp *ephemeral.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The symmetric keys and connection strings of a module.",
		Attributes: map[string]schema.Attribute{
			...
			"primary_connection_string": schema.StringAttribute{
				MarkdownDescription: "`HostName=…;DeviceId=…;ModuleId=…;SharedAccessKey=<primary key>`. Null unless `authentication_type` is `sas`.",
				Computed:            true, Sensitive: true,
			},
			"secondary_connection_string": schema.StringAttribute{
				MarkdownDescription: "`HostName=…;DeviceId=…;ModuleId=…;SharedAccessKey=<secondary key>`. Null unless `authentication_type` is `sas`.",
				Computed:            true, Sensitive: true,
			},
		},
	}
