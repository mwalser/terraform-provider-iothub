...
| Device Provisioning Service data plane (enrollments) | Different host (`*.azure-devices-provisioning.net`), different spec. Same gap, same pattern — a natural **sister scope** for a separate provider that reuses this one's patterns. |
...
| `provider::iothub::edge_manifest` | function | none — builds `modules_content` for the edge deployment resource and the set-modules action (§6.6) |
...
### 6.6 `provider::iothub::edge_manifest` — building `modules_content` (1.1, 2026-08-17)
...
modules_content = provider::iothub::edge_manifest({
  ...
  modules = {
    tempSensor = { image = "…", create_options = { HostConfig = { Binds = ["/data:/data"] } }, env = { SendInterval = 5 }, desired = { SendData = true } }
  }
  routes            = { upstream = "FROM /messages/* INTO $upstream", alerts = { route = "…", priority = 0, time_to_live_secs = 60 } }
  store_and_forward = { time_to_live_secs = 3600 }
  layered           = false
})
```

**Why a function.** The manifest's pain is its shape, not the resource: `$`-prefixed and dotted keys, `createOptions` as a JSON string inside JSON (double `jsonencode`) with the `createOptions01…` chunk convention, `env` values wrapped as `{value}`, `schemaVersion`/`runtime`/system-module boilerplate, and a different nesting for layered deployments. A builder is a **function** and not a data source because Terraform re-applies argument marks to a function's result (`internal/providers/functions.go` builds cty functions without `AllowMarked`), so a sensitive registry password makes the whole `modules_content` sensitive exactly as with `jsonencode` — a data source's computed attribute comes back unmarked and would print the password in every replacement diff. A function also leaves nothing in state, composes inside `jsonencode`/`templatefile`, and — with a dynamic parameter — takes `create_options` and `desired` as native objects. Verified (plugin-testing, Terraform 1.15.8): sensitive input → sensitive `modules_content` in the plan; an unknown input → the result is unknown until apply, like `jsonencode`.

**Contract.** One dynamic argument, strict: unknown keys, wrong types, invalid names and values are errors naming the path (`modules["m"].restartPolicy: unknown key; accepted: …`). No merge and no override hook — a manifest the object cannot express is written as JSON (`file()`/`jsonencode()`), which produces the same string; both paths stay first-class. The output is compact JSON with sorted keys (Terraform records function results in the plan and checks them again at apply, so the encoding must be deterministic). Fixed choices, agreed 2026-08-17: `$edgeAgent` and `$edgeHub` schema version **1.1** (the lowest that expresses everything emitted; the agent rejects anything above 1.1, hub 1.2/1.3 only add the removed MQTT broker and the unverified `integrity` block); **images required, no default** (a baked-in tag would replace every deployment on the provider upgrade that bumps it); template-parity defaults (`status = running`, `restart_policy = always` — the agent's own default is `on-unhealthy` — `storeAndForwardConfiguration.timeToLiveSecs = 7200` always emitted because a missing TTL is zero, `edgeHub` `createOptions` = the 5671/8883/443 port bindings every reference manifest carries, `createOptions = "{}"` when unset); `minDockerVersion`/`loggingOptions` dropped (the agent never reads them); `createOptions` chunked into 512-character pieces only when longer (the agent's own writer: `TwinValueMaxSize = 512`, `TwinValueMaxChunks = 100`); route check limited to the `FROM … [WHERE …] INTO $upstream | BrokeredEndpoint("/modules/<m>/inputs/<i>")` skeleton. Not covered: `mqttBroker`, `integrity`, `settings.digest`, `configuration.id`, schema-version inputs, partial module-twin overrides in layered mode (`properties.desired.<key>`).

**Runtime facts the design rests on** (IoT Edge source at tag 1.6.1, 2026-08-17): the agent accepts `schemaVersion` 1.0/1.1 only and applies no per-version feature gating (`EdgeAgentConnection.ValidateSchemaVersion`); `type` is mandatory and `docker` on `runtime`, `systemModules.*` and `modules.*` (`TypeSpecificSerDe`); `systemModules.edgeAgent` ignores `status`/`restartPolicy`/`startupOrder` (`EdgeAgentDockerModule`), `edgeHub` requires `status` and honours `startupOrder`; `registryCredentials.<label>` needs non-blank `address`/`username`/`password`, matched by address; `createOptions` may be a string or an object, chunks `createOptions`, `createOptions01`… are concatenated without separator, gapless, ≤ 100 (`JsonEx.ChunkedValue`); Docker options are bound to the agent's own `CreateContainerParameters` with `[JsonExtensionData]` pass-through at every level; the agent validates no module names (Microsoft's schema: `^[a-zA-Z0-9_-]+$`; twin key rules apply anyway); layering is a hub-side merge, the agent only sees merged twins. The hub accepts `schemaVersion` 1.0–1.3 exactly (`EdgeHubConfigParser`), requires `routes` from 1.1 on (`{}` allowed) and `storeAndForwardConfiguration` in every version (`timeToLiveSecs` missing → `TimeSpan.Zero`, negative → never expire; `storeLimits.maxSizeBytes` optional), route object form (`priority` 0–9, `timeToLiveSecs`, 0 = use the global TTL) from 1.1 on, sinks `$upstream` or `BrokeredEndpoint` with exactly four path segments; `mqttBroker` is parsed but dead (broker removed 2022). Microsoft's published schemas (schemastore `azure-iot-edgeagent-deployment-1.1.json`, `azure-iot-edgehub-deployment-1.2.json`) add the naming rules used for env vars (`^[^\+#$\s\.]+$`) and route/credential names (`^[^\.\$# ]+$`).

Verified live (2026-08-17, F1 hub): a base manifest built by the function, including `createOptions` split across two 512-character chunks, round-trips exactly (`jsondecode(read-back) == jsondecode(output)`, second plan empty); a layered manifest with routes only — `{"$edgeAgent": {}, "$edgeHub": {"properties.desired.routes.<name>": …}}` — is accepted; `iothub_set_edge_modules` applies the built manifest to an edge device (`$edgeAgent` twin shows `schemaVersion` 1.1).
...
- **An import job reports `completed` with `failureReason: null` even when individual lines failed** — per-line errors are written only to `importErrors.log` in the output container (`{"deviceId":…,"errorCode":409001,"errorStatus":…}`). The action's contract is deliberately limited to what the IoT Hub API reports: it succeeds when the job is `completed`, and its completion message and documentation state that per-line failures, if any, are in `<output container>/importErrors.log`. Reading that blob would mean a storage token scope, a storage role for the Terraform principal and blob parsing — another service's data plane, outside this provider's boundary (§2, §14 row 18). Note for users authoring import files: every line requires `status`, including `delete` and `updateTwin` lines.
- Cancel (`DELETE /jobs/{id}`) works instantly on a running import. `GET` of an unknown import/export job returns 404 `JobNotFound`, but `DELETE` of an unknown or older terminal job can return 500; the action therefore reads first and treats a terminal record as success instead of sending `DELETE`. Default blob names are `devices.txt`, `configurations.txt`, `importErrors.log`; exports write one `ExportImportDevice` JSON per line, modules on their own lines, and with `exclude_keys_in_export` a plain-text warning as the *first line* (the blob is then not pure NDJSON).

Verified scheduled-job mechanics: create answers `status: queued`; a duplicate `jobId` is 409 `JobAlreadyExists`; cancelling a queued or scheduled job answers `cancelled` but the next `GET` may still say `scheduled` (poll to a terminal state); a job already `running` rejects cancellation with 405 `OperationNotAllowedInCurrentState`; **`GET /jobs/v2/{unknown}` returns 200 with `{"type":"unknown","status":"unknown"}`** — the data source and cancel action treat that as not found. The cancel action reads first because cancelling an already completed job also returns 405.
...
One provider-defined function, `edge_manifest` (§6.6, 1.1): it builds a document, not an interpolation. Connection strings come from the ephemeral credentials resource; anything else is plain string interpolation.
...
- **Time-budgeted retries instead.** `429` (`ThrottlingBacklogTimeout`/`ThrottlingException`) and `503 ServerBusy/ServiceUnavailable` are retried for up to **20 minutes per request** — a fixed budget, the same for resources, data sources and ephemeral resources; actions additionally bound their whole invocation with `timeout` (default 1 h for job actions, 10 min for the others), because they wait for the hub. Resources have no `timeouts {}` block: it only ever governed this retry budget, its default served everyone, and a large fleet on a small tier is applied in slices anyway (dropped 2026-08-16, §14 #22) — never a fixed retry *count*, which would still fail applies at 1.67 ops/s. Verified: the hub sends **no `Retry-After` header** on 429; the body says *"The request has been throttled. Wait 10 seconds and try again. Operation type: ConfigurationWrite"* — so the policy honours `Retry-After` if it ever appears and otherwise starts at **10 s** with full jitter (cap 60 s). Implemented as an own `azcore` pipeline policy (azcore's count-based policy is disabled). Errors azcore marks non-retriable — above all a credential that cannot get a token — are not retried, so a wrong secret or an expired login fails at once.
...
- **ETag-gated refresh** (for `iothub_device` *and* `iothub_module`, no knob, lossless): the twin (`GET /twins/{id}[/modules/{mid}]`, 100/s) carries `deviceEtag`, which is verified to equal the identity ETag (module twin → module identity ETag), to move on every identity write and to stay put across connects and telemetry. Refresh reads the twin first; if `deviceEtag` equals the ETag in state the identity — keys included — is unchanged and only the volatile fields the twin carries (`lastActivityTime`, `cloudToDeviceMessageCount`) are refreshed from it; the twin has no `connectionStateUpdatedTime`, so a changed `connectionState` (and any changed ETag, or an unreadable twin) falls back to `GET /devices/{id}` (1.67/s on S1). Refresh becomes ~60× cheaper on S1 for quiet fleets. Requires `twins/read` in addition to `devices/read` (*IoT Hub Registry Contributor* alone does not have it) or, under SAS, a policy with *ServiceConnect* (verified: `registryReadWrite` answers 401 on `/twins`); a 401/403 on the first twin read marks the hub for the rest of the run and the registry is read silently. Import (no ETag in state) and the data sources always read the registry.
...
- No `timeouts {}` block on resources (fixed 20-minute retry budget per request); every action has a `timeout` (job actions default 1 h, the others 10 min, `iothub_cancel_job` 5 min).
...
| 16 | Provider functions | **Dropped** in 1.0 for interpolation; **`edge_manifest` added** in 1.1 (2026-08-17) as a builder for `modules_content` — a function, not a data source, strict, no override hook (§6.6) | Interpolation is trivial and ephemeral credentials return connection strings; the manifest builder is different: sensitivity flows through functions but not through data-source outputs, and the manifest quirks are worth hiding. The escape hatch is `file()`. |
...
| 22 | `timeouts {}` block on resources | **Dropped** (2026-08-16): fixed 20-minute retry budget per request, documented once | The block only ever bounded throttling retries — a device `PUT` takes under a second — so it was a knob whose default served everyone; on configurations, deployments and twins nobody would ever set it, and a fleet large enough to need more than 20 minutes on S1 is applied in slices anyway. Actions keep `timeout` because they genuinely wait (jobs, device answers). Removes 24 attributes, the timeouts plumbing in six resources and a dependency. |
...
| Digital Twin | Invoke Root / Component Command | `POST /digitaltwins/{id}[/components/{path}]/commands/{name}` | not exposed (SAS only — the service rejects Entra ID tokens; the equivalent direct method `<component>*<command>` works under both modes, see §14 #20) |
...
|---|---|
| `iothub_device`, `iothub_module` | `devices/read`, `devices/write`, `devices/delete` (+ `twins/read` for the cheap ETag-gated refresh, §11.2; optional) |
...
| ETag churn | Identity ETag, twin ETag and `deviceEtag` unchanged across connect/disconnect/8 telemetry messages; only `connectionState`/`connectionStateUpdatedTime` moved. An identity write (`PUT /devices/{id}`, status change) moves `deviceEtag` immediately while the twin ETag stays; the module twin's `deviceEtag` is the module identity ETag. The twin's identity mirror carries `connectionState`, `lastActivityTime`, `cloudToDeviceMessageCount`, `status(Reason)`, `statusUpdateTime` — but **not** `connectionStateUpdatedTime` or `generationId`. Under SAS, `GET /twins/…` needs *ServiceConnect* (`registryReadWrite` → 401 `IotHubUnauthorizedAccess`; `service` → 200) | Conflict-inspection retry branch is rare; ETag-gated refresh implemented as in §11.2 (fallback on changed connection state; 401 and 403 both mark the hub) |
...
| Twin validation | Keys: no control chars, `.`, `$`, `#`, space (empty and unicode keys accepted), ≤ 1024 bytes; string values ≤ 4096 bytes; objects nest ≤ 10 deep (a scalar may sit inside the 10th); `null` rejected anywhere, also inside arrays; integers range-checked to ±2⁵²−1; `1` and `1.0` round-trip verbatim (service messages are precise) | Mirrored as client-side validators in `internal/twinpatch`; numbers compared by value |
...
| Modules | Create 200; duplicate 409 `ModuleAlreadyExistsOnDevice`; `$`-prefixed rejected (400); delete needs `If-Match`; edge devices get `$edgeAgent`/`$edgeHub` (managedBy `iotEdge`, auth `none`); modules have no `status`; `PUT` is a full replace — omitted `managedBy` clears it, omitted `authentication` mints new keys; the module twin's `deviceEtag` is the module identity ETag; **under SAS authentication, module identity operations on a device that does not exist answer 401 `IotHubUnauthorizedAccess` (401002) instead of 404** (Entra ID: 404; module *twin* paths: 404 in both); **likewise, under SAS (even `iothubowner`) module identity operations — GET, list, PUT, DELETE — on a *disabled* device answer 401**, while the module twin (GET/PATCH) and Entra ID keep working (verified 2026-08-15) | As specified; `iothub_module` shares the identity/auth code with `iothub_device`; the client probes the device on a module 401 and reports a missing device as 404 and a disabled device with an explanatory error (`ModuleOnDisabledDevice`); the twin-first refresh keeps unchanged modules on disabled devices readable under SAS |
